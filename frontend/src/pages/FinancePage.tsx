  ...
  return (
    <div>
        ...
        <>
          ...
          {stats.perUser.length > 0 && (
            <div className="glass mb-8 rounded-2xl p-4 sm:p-5">
              ...
              {/* Desktop table */}
              <div className="hidden sm:block overflow-hidden rounded-xl border border-white/40">
              ...
              <div className="space-y-2 sm:hidden">
                {stats.perUser.map((u) => (
                  <div key={u.username} className="rounded-xl bg-white/40 px-4 py-3">
                    <p className="text-sm font-semibold text-text-bright mb-2">{u.username}</p>
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div>
                        <p className="text-[10px] text-text-secondary">Einnahmen</p>
                        <p className="font-mono text-xs font-medium text-status-customer">{formatCurrency(u.income)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-text-secondary">Ausgaben</p>
                        <p className="font-mono text-xs font-medium text-status-churned">{formatCurrency(u.expense)}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-text-secondary">Gewinn</p>
                        <p className={`font-mono text-xs font-medium ${u.profit >= 0 ? "text-accent" : "text-status-churned"}`}>{formatCurrency(u.profit)}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <div className="glass mb-6 rounded-2xl p-4 sm:p-5">
        ...
        <form onSubmit={handleCreate} className="space-y-3 sm:space-y-0 sm:flex sm:flex-wrap sm:gap-3">
          <div className="grid grid-cols-2 gap-3 sm:flex sm:gap-3">
            ...
            <input type="number" step="0.01" min="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Betrag" required className="glass-input rounded-xl px-3 py-2.5 text-sm text-text-bright sm:w-32" />
          </div>
          <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Beschreibung" required className="glass-input w-full rounded-xl px-3 py-2.5 text-sm text-text-bright sm:min-w-0 sm:flex-1" />
          <div className="grid grid-cols-2 gap-3 sm:flex sm:gap-3">
...
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="glass rounded-2xl p-3 sm:p-4">
      ...
      <p className={`mt-1 font-mono text-base font-bold sm:text-2xl ${color}`}>{formatCurrency(value)}</p>
    </div>
  );
}
